...
double fitness_function(const Model* model, TargetFunc func, int N, int M) {
    double mse = 0.0;
    for (int i = 0; i < N; i++) {
        double u1 = rand_double(U1_MIN, U1_MAX);
        double u2 = rand_double(U2_MIN, U2_MAX);
...
void crossover(const Model* parent1, const Model* parent2, Model* child1, Model* child2, double crossover_rate, int M) {
    ...
    *child2 = *parent2;
    ...
    if (rand_double(0.0, 1.0) < crossover_rate) {
        int crossover_point = 1 + rand() % (M - 1); // 1 to M-1
...
void mutation(Model* individual, double mutation_rate, double mutation_strength, int M) {
    for (int i = 0; i < M; i++) {
        if (rand_double(0.0, 1.0) < mutation_rate) {
            individual->gaussians[i].w += mutation_strength * rand_normal();
        }
        if (rand_double(0.0, 1.0) < mutation_rate) {
            individual->gaussians[i].c[0] += mutation_strength * rand_normal();
        ...
        if (rand_double(0.0, 1.0) < mutation_rate) {
            individual->gaussians[i].c[1] += mutation_strength * rand_normal();
        ...
        if (rand_double(0.0, 1.0) < mutation_rate) {
            individual->gaussians[i].sigma[0] += mutation_strength * rand_normal();
        ...
        if (rand_double(0.0, 1.0) < mutation_rate) {
            individual->gaussians[i].sigma[1] += mutation_strength * rand_normal();
